...
const Why = ({ navigation }: AuthNavProps<"Why">) => {
  const [userWhy, setWhy] = useState("");
            ...
            data.map((dt, idx) => {
              return (
                <TouchableOpacity
                  ...
                  style={[
                    styles.card,
                    userWhy == dt ? { backgroundColor: "#D9F4FB" } : null,
                    idx === 0 ? topRadius : null,
                    idx === 6 ? bottomRadius : null,
                  ]}
                  key={dt}
                >
                  <Text
                    style={[
                      styles.cardText,
                      userWhy == dt ? { color: "#24A9DB" } : { color: "#000" },
                    ]}
                  >
                    {dt}
                  </Text>
